...
class EvfSam2Model(PreTrainedModel):
    ...
    def forward(
        self,
        images: torch.FloatTensor,                 # torch.Size([2, 3, 1024, 1024])  # for SAM
        images_evf: torch.FloatTensor,             # torch.Size([17, 3, 224, 224])   # for EVF
        input_ids: torch.LongTensor,               # (17, 13)
        attention_masks: torch.LongTensor,         # (17, 13)
        offset: torch.LongTensor,                  # (3,)
        masks_list: List[torch.FloatTensor],       # List[ torch.Size([11, 424, 640]) ]  x  2
        label_list: List[torch.Tensor],            # List[ torch.Size([424, 640]) ]      x  2
        resize_list: List[tuple],                  # [[1024, 1024], [1024, 1024]]
        ...
        **kwargs,
        ...
        # mask_loss = mask_bce_loss + 5.0 * mask_dice_loss
